#!/usr/bin/env python3
"""
Step 2: Steering Validation Script
Validates discovered neurons against baseline methods.
Requires discovered neurons from Stage 1 (discover_neurons.py).
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate discovered neurons (Stage 2) - requires discovered neurons from Stage 1"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to JSON file with discovered neurons (from discover_neurons.py)"
    )
    parser.add_argument(
        "--validation_size",
        type=int,
    ...
    # Check if neurons file exists
    neurons_path = Path(args.neurons)
    
    # If relative path, check in output_dir first, then current directory
    if not neurons_path.is_absolute() and not neurons_path.exists():
        output_dir = Path(args.output_dir)
        potential_path = output_dir / neurons_path.name
        if potential_path.exists():
            neurons_path = potential_path
            logger.info(f"📂 Found neurons file in output directory: {neurons_path}")
    
    if not neurons_path.exists():
        logger.error(f"❌ Neurons file not found: {args.neurons}")
        logger.error(f"   Checked: {neurons_path}")
        if not neurons_path.is_absolute():
            logger.error(f"   Also checked: {Path(args.output_dir) / neurons_path.name}")
        logger.error(f"   Please run discovery first: python discover_neurons.py --model {args.model} --save_path {args.neurons}")
        sys.exit(1)
    ...
    # Load discovered neurons
    discovered_neurons = load_discovered_neurons(neurons_path)
    
    # Run validation
    logger.info("\n" + "="*80)
    logger.info("VALIDATION OF DISCOVERED NEURONS (Stage 2)")
    logger.info("="*80)
    ...
    # Save results (import from ablation_3 main function logic)
    from ablation_3 import generate_protocol_summary, create_neuron_importance_visualization
